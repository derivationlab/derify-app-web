...
const TradingVolume: React.FC = () => {
  ...
  const [time2, setTime2] = useState("1D");
  const [time3, setTime3] = useState("1D");
  ...
  const onOptionChange = (value: string) => {
    ...
    loadTradeDataAction(dispatch)
      .then(data => {
        ...
        const xaxis = [];
        const tradAmSeries: { stack: string; data: number[] } = {
          stack: "trading amount",
          data: [],
        };
        const tradFeeSeries: { stack: string; data: number[] } = {
          stack: "trading fee",
          data: [],
        };
          ...
          <Col className="chart-out-wrapper">
            <Row>
              ...
              <Col flex="50%">
                <div>
                  <Badge color="#475FFA" />
                  {formatMessage({ id: "Data.Data.Trade.TradFeeEarning" })}
                  （24h）
                </div>
                <div style={{ margin: "4px 0", paddingLeft: "14px" }}>
                  <span>
