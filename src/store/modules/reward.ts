...
const actions = {
  loadEarningData(trader: string) {
    return async (commit: Dispatch) => {
      ...
      try {
        ...
        console.error("getTraderVariables error", e);
      }
      ...
      const earningData = {
        ...pmrReward,
        accountData: { ...traderVariable },
        bondInfo,
        edrfInfo,
      };
      commit({ type: "reward/updateState", payload: earningData });
      commit({
        type: "reward/updateWallet",
        payload: { bdrfBalance, edrfBalance: edrfBalance },
      });
  ...
  exchangeBond(
    ...
    bondAccountType: BondAccountType
  ) {
    return async (commit: Dispatch) => {
      if (!trader) {
        return;
  ...
  getExchangeBondSizeUpperBound(
    ...
    return async (commit: Dispatch) => {
      ...
      const contract = web3Util.contract(trader);
      const exchangeBondSizeUpperBound = await contract.getExchangeBondSizeUpperBound(
        { trader, bondAccountType }
      );

      commit({
        type: "reward/updateState",
        payload: { exchangeBondSizeUpperBound },
      });
    ...
    return async (commit: Dispatch) => {
      ...
      pagenation.records.forEach((item: TradePMRBalance) => {
        results.push({
          id: item.id,
          amount: item.amount,
          amoutToken: getUSDTokenName(),
          balance: item.balance,
          balanceToken: getUSDTokenName(),
          time: item.event_time,
          type:
            item.pmr_update_type === 0
              ? "Rewards.Mining.History.Earning"
              : "Rewards.Mining.History.Withdraw",
        });
      });
