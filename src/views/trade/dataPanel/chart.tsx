...
const Chart: React.FC<ChartModalProps> = props => {
  ...
  const timeGap =
    timeOptions.find(item => {
      return typeof item === "object" && item.value === bar;
    }) || timeOptions[0];

  const beforeStr = after - timeGap.time * (limit || 1);
  const [before, setBefore] = useState(beforeStr);

  const updateChartKlineData = (
    token: string,
    bar: string,
    after: number,
    before: number,
  ...
  const onMouseMove = (e: MouseEvent) => {
    ...
    afterTimestamp =
      afterTimestamp -
      Math.ceil((deltaX / chartEle.clientWidth) * limit * timeOption.time);
  ...
  const onMouseUp = (e: MouseEvent) => {
    ...
    const deltaTime = Math.ceil(
      (deltaX / chartEle.clientWidth) * limit * timeGap.time
    );
