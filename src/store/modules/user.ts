...
export async function asyncInitWallet(): Promise<UserState> {
  ...
  window.ethereum.chainId = await window.ethereum.request({
    method: "eth_chainId",
  });
  window.ethereum.ethAccounts = await window.ethereum.request({
    method: "eth_accounts",
  });

  if (window.ethereum.ethAccounts.length > 0) {
    window.ethereum.selectedAddress = toChecksumAddress(
      window.ethereum.ethAccounts[0]
    );
  }

  window.ethereum.networkVersion = await window.ethereum.request({
    method: "net_version",
  });
...
export async function getWallet(): Promise<UserState> {
  ...
  const chainEnum = networkMap.hasOwnProperty(chainId)
    ? networkMap[chainId]
    : new ChainEnum(chainId, "unkown");
  let bindBrokerId = "";
  let selfBrokerId = null;
  let traderBroker = null;

  const isLogin =
    wethereum.selectedAddress && isEthum && !isLogout() && !chainEnum.disabled;
...
export function setHasBroker(trader: string, hasBroker: boolean) {
  window.localStorage.setItem(
    "broker.bind",
    JSON.stringify({ trader, hasBroker })
  );
}
...
const actions = {
  getBalanceOfDUSD(trader: string, token: string) {
    return async (commit: Dispatch) => {
      ...
      const balanceOf = await web3Utils
        .contract(trader)
        .balanceOf(trader, token);
      commit({
        type: "user/updateState",
        payload: { balanceOfDUSD: balanceOf },
      });
      return balanceOf;
    };
  },
  ...
  logout() {
    return async (commit: Dispatch) => {
      ...
      commit({
        type: "user/updateState",
        payload: { isLogin: false, selectedAddress: "", trader: "" },
      });
      return true;
    };
  },
