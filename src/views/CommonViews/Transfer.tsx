...
const Transfer: React.FC<TransferProps> = props => {
  const loadAccountStatus = useSelector(
    (state: RootStore) => state.app.reloadDataStatus.account
  );
  ...
  const getMaxAmount = useCallback(
    (transferData: TransferData, operateType: TransferOperateType) => {
  ...
  const loadTransferData = useCallback(
    async transferType => {
      ...
      try {
        transferData.balanceOfWallet = await contract.balanceOf(
          trader,
          Token.DUSD
        );
      } catch (e) {
  ...
  const checkAmount = useCallback(
    (amount, transferData, transferType) => {
      const checkRet = checkNumber(
        amount,
        fromContractUnit(getMaxAmount(transferData, transferType))
      );
  ...
  const changeTransferType = useCallback(() => {
    ...
    let newTransferType:
      | TransferOperateType.withdraw
      | TransferOperateType.deposit;
  ...
  useEffect(() => {
    if (visible) {
      ...
      operateType === TransferOperateType.deposit
        ? form.setFieldsValue({
  ...
  const doTransfer = useCallback(() => {
    ...
    if (transferType == TransferOperateType.withdraw) {
      ...
      props.closeModal();
      const action = contractModel.actions.withdrawAccount(
        trader,
        toContractUnit(amount)
      );
      action(dispatch)
        .then(data => {
      ...
      props.closeModal();
      DerifyTradeModal.pendding();
      const action = contractModel.actions.depositAccount(
        trader,
        toContractUnit(amount)
      );
      action(dispatch)
      ...
      <Row>
        <Col flex="100%">
          ...
          <Form layout={"vertical"} form={form}>
            <Form.Item
              label={<FormattedMessage id="Trade.Account.Transfer.From" />}
              name="from"
            >
              <Input readOnly={true} />
            </Form.Item>
            <Form.Item
              label={<FormattedMessage id="Trade.Account.Transfer.To" />}
              name="to"
            >
              <Input readOnly={true} />
            </Form.Item>
            <Form.Item
              label={<FormattedMessage id="Trade.Account.Transfer.Size" />}
            >
              <Input
                bordered={false}
                addonAfter={getUSDTokenName()}
                value={amount}
                onChange={onchange}
              />
            </Form.Item>
