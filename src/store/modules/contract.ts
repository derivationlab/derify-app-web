...
const reducers = createReducer(state, {
  ...
  "contract/ADD_POSITION_DATA"(state: ContractState, { payload }) {
    ...
    if (payload.positionData.orderPositions) {
      ...
      payload.positionData.orderPositions.forEach(
        (position: OrderPositionData, index: number) => {
          const key =
            position.timestamp +
            "." +
            position.token +
...
const actions = {
  ...
  updateTokenSpotPrice(trader: string, token: string) {
    return async (commit: Dispatch) => {
      ...
      web3Utils
        ...
        .then(spotPrice => {
          commit({
            type: "contract/UPDATE_PAIRS",
            payload: [{ num: fromContractUnit(spotPrice), key: matchPair.key }],
          });
        });
    };
  },
  ...
  getCloseUpperBound(trader: string, token: string, side: SideEnum) {
    return async (dispatch: Dispatch) => {
      ...
      if (side !== SideEnum.HEDGE) {
        closeUpperBound = await web3Utils
          .contract(trader)
  ...
  openPosition({
    ...
    return async (dispatch: Dispatch) => {
      ...
      const ret = await web3Utils
        .contract(trader, brokerId)
        .openPosition(params);
  ...
  cancleOrderedPosition(params: {
    trader: string;
    token: string;
    closeType: CancelOrderedPositionTypeEnum;
    side: SideEnum;
    timestamp: number;
  }) {
    return async (dispatch: Dispatch) => {
  ...
  cancleAllOrderedPositions(trader: string) {
    return async (dispatch: Dispatch) => {
      ...
      const ret = await web3Utils.contract(trader).cancleAllOrderedPositions();
  ...
  loadHomeData({
    ...
    return async (commit: Dispatch) => {
      const data = {
        ...
        traderOpenUpperBound: { amount: 0, size: 0 },
        sysOpenUpperBound: 0,
      };
      if (!trader) {
        return {};
      ...
      const updateAllPairPriceAction = self.updateAllPairPrice(
        trader,
        curPair.address
      );
      ...
      data.positionChangeFeeRatio = await contract.getPositionChangeFeeRatio(
        token
      );
      commit({ type: "contract/SET_CONTRACT_DATA", payload: { ...data } });
      ...
      data.sysOpenUpperBound = await contract.getSysOpenUpperBound({
  ...
  getSysOpenUpperBound(trader: string, side: SideEnum, token: string) {
    return async (commit: Dispatch) => {
      ...
      if (side !== SideEnum.HEDGE) {
        sysOpenUpperBound = await contract.getSysOpenUpperBound({
          token: token,
          side,
        });
      }
      ...
      commit({
        type: "contract/SET_CONTRACT_DATA",
        payload: { sysOpenUpperBound },
      });
  ...
  getSysCloseUpperBound(trader: string, side: SideEnum) {
    return async (commit: Dispatch) => {
      ...
      if (side !== SideEnum.HEDGE) {
        sysCloseUpperBound = await contract.getSysCloseUpperBound({
          token: curPair.address,
          side,
        });
      }
  ...
  loadPositionData(
    ...
    { positionData: PositionDataView | null; pair: TokenPair | null }[]
  > {
    return async (
      commit: Dispatch
    ): Promise<
    ...
    > => {
      ...
      const positionDatas: {
        positionData: PositionDataView | null;
        pair: TokenPair | null;
      }[] = [];
      for (let idx = 0; idx < state.pairs.length; idx++) {
        ...
        const positionDataView: PositionDataView = await contract.getTraderAllPosition(
          trader,
          pairItem.address
        );
        const positionData: {
          positionData: PositionDataView | null;
          pair: TokenPair | null;
        } = { positionData: positionDataView, pair: pairItem };
  ...
  }): (commit: Dispatch) => Promise<OpenUpperBound> {
    return async (commit: Dispatch) => {
      if (!trader) {
        return { amount: 0, size: 0 };
      }
      ...
      const update = Object.assign({}, state.contractData, {
        traderOpenUpperBound: data,
      });
