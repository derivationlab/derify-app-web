import {
  bindBroker,
  BrokerInfo,
  getbrokerBindTraders,
  getBrokerByBrokerId,
  getBrokerByTrader,
  getBrokerList,
  getBrokerRewardHistory,
  ...
  getTraderBrokerInfo(trader: string) {
    return async (commit: Dispatch) => {
          ...
          expireDate.setTime(
            expireDate.getTime() +
              1000 *
                ...
                60 *
                24 *
                fromContractUnit(accountInfo.validPeriodInDay)
          );
      ...
      try {
        ...
        if (brokerInfo !== null) {
          brokerAccountInfo.reference =
            getWebroot() + "/broker/" + brokerInfo.id;
          Object.assign(brokerAccountInfo, brokerInfo);
        }
        try {
          brokerAccountInfo.todayReward = toContractNum(
            await getBrokerTodayReward(trader)
          );
        } catch (e) {
  ...
  getBrokerBalance({
    ...
    return async (commit: Dispatch) => {
      ...
      if (accountType === BondAccountType.WalletAccount) {
        ...
        commit({
          type: "broker/updateWallet",
          payload: { walletEdrfBalance: amount },
        });
      } else {
        const accountInfo = await contract.getStakingInfo(trader);
        commit({
          type: "broker/updateWallet",
          payload: { derifyEdrfBalance: accountInfo.edrfBalance },
        });
      }
    };
  },
