...
function Operation() {
  ...
  const updateMaxAmount = useCallback(
    (openType, limitPrice, leverage) => {
      ...
      const getTraderOpenUpperBoundAction =
        contractModel.actions.getTraderOpenUpperBound(params);
      const tokenNew = token;
      getTraderOpenUpperBoundAction(dispatch)
        .then(data => {
  ...
  const doOpenPositionConfirm = useCallback(
    (side: number) => {
      ...
      DerifyErrorNotice.error(null);
      const tokenSize = calculatePositionSize(
        size,
        token,
        traderOpenUpperBound,
        sliderVal
  ...
  useEffect(() => {
    ...
    const onOpenPositionAction = ContractModel.actions.onOpenPosition(
      trader,
      () => {
          ...
          {openType === OpenType.MarketOrder ? (
            ...
            <Col flex="100%" className="derify-input">
              <Input
                size="large"
                value={limitPrice}
                onChange={({ target: { value } }) => onLimitPriceChange(value)}
              />
            </Col>
          )}
        </Row>
